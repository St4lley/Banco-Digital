...
const criarConta = (req,res) =>{
    ...
    if(!nome||!cpf||!data_nascimento||!telefone||!email||!senha){
        return res.status(400).json({mensgame:"Aesencia de informacoes obrigatorias"})
    }
...
const alterarConta=(req,res) =>{
    const {numero,nome,cpf,data_nascimento,telefone,email,senha} = req.body
    const {numeroConta} = req.params

    const numero_conta = contas.findIndex((conta)=>conta.numero == numeroConta)
    ...
    const verificacaoConta = contas.filter((conta)=>conta.numero != numeroConta)
...
const excluirConta =(req,res) =>{
    ...
    const numeroConta = contas.findIndex((conta)=>conta.numero == numero_conta)
...
const depositarMoney = (req,res) =>{
    ...
    if(valor <= 0){
        return res.status().json({mensagem:"O valor deve ser maior que 0 reais."})
    }
...
const sacarMoney = (req,res) =>{
    ...
    if(!numero_conta ||!valor ||!senha){
        return res.status(401).json({mensagem:"O número da conta,senha e o valor são obrigatórios!"})
    }
    ...
    if(senha !== contas[numeroConta].usuario.senha){
        return res.status(402).json({mensagem:"ACESSO NEGADO,senha incorreta"})
    }
...
const transferirMoney = (req,res) =>{
    ...
    if(!numero_conta_destino || !numero_conta_origem || !valor || !senha){
        res.status(400).json({mensagem:"todas as informacoes devem ser informadas"})
    }
    ...
    //pegando o indice
    const contaOrigem = contas.findIndex((conta)=>conta.numero === numero_conta_origem)
    const contaDestino = contas.findIndex((conta)=>conta.numero == numero_conta_destino)
...
const consultarSaldo = (req,res) =>{
    ...
    if(!numero_conta || !senha) {
        return res.status.json({mensagem:"Informe corretamente o numero da conta e senha"})
    }
